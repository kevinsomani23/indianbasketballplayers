...
class VibeMasterParser:
    ...
    def _scrape_all(self):
        ...
        for i, table in enumerate(tables):
            ...
            for row in table.select("tbody tr"):
                ...
                mins_raw = p_obj.get("Min", p_obj.get("Mins", "0:0"))
                if ":" in mins_raw:
    ...
    def _process_pbp(self):
        ...
        for row in self.raw_data["PBPRows"]:
            ...
            # Entity Mapping
            # 1. Try standard "Jersey Name" format
            ...
            # 2. Fallback: Try Name matching (for cases like "Amanda Maria Rocha, ...")
            if not p_name:
                # heuristic: assume "Name, Action" -> split by comma
                parts = desc.split(',')
                if len(parts) > 1:
                    candidate_name = parts[0].strip()
                    # Check if this name exists in our stats for the current team
                    if candidate_name in self.player_stats and self.player_stats[candidate_name]["Team"] == self.metadata["Teams"][team]:
                        p_name = candidate_name
                        jersey = self.player_stats[p_name]["Jersey"] # Backfill jersey
